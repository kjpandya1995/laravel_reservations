<nav x-data="{ open: false }" class="bg-white border-b border-gray-100">
  ...
  @auth
                    ...
                    @if(auth()->user()->role_id === \App\Enums\Role::GUIDE->value)
                        <x-nav-link :href="route('guide-activity.show')" :active="request()->routeIs('guide-activity.*')">
                            {{ __('My Activities') }}
                        </x-nav-link>
                    @endif
                @endauth
                ...
                <x-dropdown align="right" width="48">
                    <x-slot name="trigger">
                        <button class="inline-flex items-center px-3 py-2 border border-transparent text-sm leading-4 font-medium rounded-md text-gray-500 bg-white hover:text-gray-700 focus:outline-none transition ease-in-out duration-150">
                            <div>{{ Auth::user()->name }} (ID: {{ Auth::user()->id }}, Role: {{ Auth::user()->role_id }})</div>
                    ...
                    <x-slot name="content">
                        <x-dropdown-link :href="route('profile.edit')">
                            ...
                            @if(auth()->check() && auth()->user()->role_id === \App\Enums\Role::GUIDE->value)
                            <x-dropdown-link :href="route('guide-activity.show')">
                                {{ __('My Activitiesssss') }}
                            </x-dropdown-link>
                        @else 
